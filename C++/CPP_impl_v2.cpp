...
int main(int, char **) {
    ...
    constexpr float kappa = 0.5f, dx = 1.0f, dt = 0.01f;
    ...
    for (int istep = 0; istep < Nstep + 1; istep++) {
        ...
        vector<vector<float>> df_dc = mesh_periodic(
            con_mesh, Nx, Ny, dx,
            [kappa](float v_1, float v_2, float v_3, float v_4, float v_5, float v_6) {
                return df_bulk_dc(v_1) - kappa * laplacian(v_1, v_2, v_3, v_4, v_5, v_6);
            });
